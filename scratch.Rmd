...
========
>>>>>>>> 6581c668dc6b656aeb73a126bb446f0cb8c23bca:tidying_2022.Rmd
...
select(DT = contains('Date.Time'),
  ...
  dplyr::bind_rows(., filter(pre_2022, site == "legacy")) %>%
  full_join(filter(field_notes, site == "legacy"), by = c('DT','site')) %>%
  mutate(date = as_date((DT)),
         hour = hour(DT),
         year = year(DT)) %>%
  arrange(ymd_hms(DT)) %>%
   # remove data ranges where sensor was pulled out of field (THAT I KNOW OF!).
...
<<<<<<<< HEAD:scratch.Rmd
  # pass one (p1): remove any data outside the In-Situ spec range
  mutate(p1 = ifelse(Water_Temp_C > 50, NA,
              ifelse(Water_Temp_C < 0 , NA, Water_Temp_C)))
  # mutate(pre = lag(Water_Temp_C, 1),
  #        post = lead(Water_Temp_C, 1),
  #        roll = (pre+Water_Temp_C+post)/3)

means <- rollmean(legacy_temp$Water_Temp_C, k = 5, align = "center")
...
```{r}
legacy_depth <- dplyr::select(rawless_legacy, c(DT, Depth_ft, site)) %>%
  arrange(ymd_hms(DT)) %>%
  group_by(site,DT) %>%
  summarize(Depth_ft = mean((Depth_ft))) %>%
  ungroup() %>%
...
legacy_depth <- legacy_depth %>%
  mutate(Depth_ft_p1 = ifelse(DT >= ymd_hms('2022-04-06 06:00:00') & DT < ymd_hms('2022-04-12 09:30:00'), Depth_ft + abs(filter(legacy_depth, DT == ymd_hms('2022-04-12 09:15:00'))$Depth_ft - filter(legacy_depth, DT == ymd_hms('2022-04-12 09:30:00'))$Depth_ft), Depth_ft)) %>%
  mutate(Depth_ft_p1 = ifelse(DT >= ymd_hms('2022-07-22 11:30:00') & DT <= ymd_hms('2022-07-25 14:15:00'), Depth_ft_p1 + abs(filter(legacy_depth, DT == ymd_hms('2022-07-22 11:15:00'))$Depth_ft_p1 - filter(legacy_depth, DT == ymd_hms('2022-07-22 11:30:00'))$Depth_ft_p1), Depth_ft_p1)) %>%
...
compare <- legacy_depth %>%
  ...
  dplyr::filter(DT >= ymd_hms('2019-09-06 17:30:00'))
========
...
<<<<<<<< HEAD:scratch.Rmd
  ...
  geom_line(data=depth,aes(DT,Depth_ft*500), color='red')
========
