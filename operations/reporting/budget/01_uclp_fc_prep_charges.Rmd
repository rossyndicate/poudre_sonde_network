---
title: "Template for UCLP Monthly Invoice"
...
lapply(c("tidyverse", "readxl", "arrow", "flextable", "officer", "yaml", "here", "ross.wq.tools"), package_loader)
rm(package_loader)
# get wd
dir <- getwd()
...
sensor_field_notes <- grab_mWater_sensor_notes(mWater_api_data = mWater_data)%>%
  #fix dates
  ...
  filter(site %in% c("chd","pfal","pbr","sfm","pman","pbd") & month == month_select & year == year_select)%>%
...
sampling_field_notes <- mWater_data%>%
  ...
  mutate(date = as.Date(date),
    month = month(date),
         year = year(date),
         site = tolower(site))%>%
  #filter for UCLP sites and the month and date desired
  filter(site %in% c( "chd","pfal","pbr","sfm","pman","pbd") & month == month_select & year == year_select)%>%
  select(date,crew, site, cals_performed, DT_round,visit_type, visit_comments)


#find number of site visits
  monthly_visits <- bind_rows(sensor_field_notes, sampling_field_notes)%>%
    group_by(site)%>%
    summarise(visits = length(unique(date)))
    
  #find all the calibration dates for each site
  cal_dates <-sensor_field_notes%>%
  ...
  notes_final <- tibble(site =  c("chd","pfal","pbr","sfm","pman","pbd") )%>%
    ...
    mutate(`Site Visits` = ifelse(is.na(visits), 0, visits),
           ...
           Site = toupper(site),
           Site = factor(Site, levels = c("CHD","PFAL","PBR", "SFM","PMAN","PBD") )) %>%
  ...
  if(full == T){
    field_notes <- bind_rows(sensor_field_notes, sampling_field_notes)%>%
...
write_csv(field_summary, paste0(dir, "/docs/uclp_dss/docs/monthly_budget/field_summary/field_summary_",month_select, "_", year_select,".csv"))
...
senior_staff <- read_excel(path = paste0(dir,"/docs/pwqn/docs/monthly_budget/pwqn_budget_reporting.xlsx"), sheet = "staff_timeclock")%>%
  ...
  filter(month == month_select & year == year_select & project == "UCLP")%>%
...
mutate(hourly_billed = round(hourly_billed, 2),
      ...
      include = case_when(#remove staff/intern time paid for by Extension
                          Extension == TRUE ~ "No",
                          TRUE ~ "Yes"))%>%
...
#view all data
print(all_data)
sum(all_data$Cost[!all_data$Category %in% c("Equipment", "Travel")])
```
...
# write to csv for 02 script
write_csv(all_data, paste0(dir,"/docs/uclp_dss/docs/monthly_budget/charges_final/charges_final_", month_select, "_", year_select,  ".csv"))
