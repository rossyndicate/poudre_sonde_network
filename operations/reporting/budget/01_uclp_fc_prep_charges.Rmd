...
lapply(c("tidyverse", "readxl", "arrow", "flextable", "officer", "yaml", "here", "fcw.qaqc"), package_loader)
rm(package_loader)
# get wd
dir <- getwd()
...
sensor_field_notes <- grab_mWater_sensor_notes(mWater_api_data = mWater_data)%>%
  ...
  filter(site %in% c("joei","cbri", "chd","pfal","pbr","sfm","pman","pbd") & month == month_select & year == year_select)%>%
...
#find number of site visits
  ...
  notes_final <- tibble(site =  c("joei","cbri", "chd","pfal","pbr","sfm","pman","pbd") )%>%
    ...
    mutate(`Site Visits` = ifelse(is.na(visits), 0, visits),
           ...
           Site = toupper(site),
           Site = factor(Site, levels = c("JOEI", "CBRI", "CHD","PFAL","PBR", "SFM","PMAN","PBD") )) %>%
