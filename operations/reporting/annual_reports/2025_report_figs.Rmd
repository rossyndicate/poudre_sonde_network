...
files <- tibble(filename = list.files(here("data", "raw", "sensor","manual_data_verification","2025_cycle", "hydro_vu_pull", "flagged_data"), full.names = TRUE))%>%
  filter(grepl(pattern = params, filename))%>%
  #remove extras
  filter(!grepl(pattern = "Level|MV", filename))
...
# read in files
sensor_data <- map_dfr(files$filename, ~read_parquet(.x)) %>%
  #turn into individual dataframes by site and parameter
  # split(f = list(.$site, .$parameter), sep = "-") %>%
  # keep(~nrow(.) > 0)%>%
  bind_rows()%>%
...
plot_down_days <- function(sensor_df, year){

  sensor_data <- sensor_df %>%
    filter(lubridate::year(DT_round) == year)
  
  if(nrow(sensor_data) == 0){
    stop(paste("No data available for year:", year))
  }
  
down_days_all_data <- sensor_data %>%
...
counts <- down_days_all_data%>%
  filter(!is.na(auto_flag) & !grepl("suspect", auto_flag)) %>%
  filter(grepl("sonde unsubmerged|outside of sensor specification range", auto_flag) | #looking for sensor malfunctions or when a sensor is unsubmerged
           !is.na(mal_flag)| #looking for any inputted malfunction notes
           (parameter == "Turbidity" & grepl("drift", auto_flag) & mean >= 40) # looking for excessive drift (ie high turbidity values due to biofouling)
         ) %>%
  filter(!is.na(mean)) %>%
...
message(paste0("The PWQN was down for ", nrow(counts), " site days across the ", length(site_order), " sites."))
 ...
 ggplot() +
  ...
  labs(x = "Date", y = NULL,
       title = paste0("PWQN Down Days for ", year)
       #subtitle = paste0("The PWQN was down for ", nrow(counts), " site days across the ", length(site_order), " sites.")
       ) +
  plot_theme
...
ggsave(paste0(fig_dir,"down_days_2025.png"), plot = down_day_plot_2025, width = 10, height = 8, units = "in", dpi = 300)
```
