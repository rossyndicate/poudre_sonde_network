...
if(verified){
  ...
  sensor_data_clean <- sensor_data %>%
    filter(
      ...
      !(parameter == "DO" & mean < 2), # remove DO < 2 mg/L (sensor error)
      !(mean == 0) # remove instances of 0 values
    ) %>%
    mutate(mean = case_when(
      parameter == "Turbidity" & mean > 1000 ~ 1000, # cap turbidity at 1000 NTU
...
hist_data_clean <- read_parquet(file = "data/collated/sensor/compiled_ross_sensor_data_2026-05-18.parquet")%>%
  filter(site %in% unique(sensor_data_clean$site))%>%
  distinct(site, parameter, DT_round, hourly_median)%>%
  select(site, parameter, DT_round, mean = hourly_median)%>%
  mutate(date = as.Date(DT_round))%>%
  mutate(doy = yday(date))%>%
  summarise(median = median(mean, na.rm = TRUE),
            max = max(mean, na.rm = T),
            min = min(mean, na.rm = T),
            .by = c("doy", "site", "parameter"))%>%
  mutate(    plot_date = as.POSIXct(paste("2026", doy), format = "%Y %j"),
             max = ifelse(max == -Inf | max == Inf , NA, max), 
             min = ifelse(min == -Inf | min == Inf, NA, min))
```
...
pull_larimer_precip_q <- function(station_id = "11523",  start_dt = "2025-03-01", end_dt = "2026-07-08"){
...
```
### Load in Larimer CO Precip/Flow data

```{r}

if(file.exists(here("data/collated/external_datasets/larimer_county_station_flow_precip_20210101_20260601.parquet"))){
  larimer_flow_precip <- read_parquet("data/collated/external_datasets/larimer_county_station_flow_precip_20210101_20260601.parquet")%>%
  ...
  all_data <- map(meta$site, ~pull_larimer_precip_q(station_id = .x, start_dt = "2021-01-01", end_dt = "2026-06-01"))
...
create_precip_map(larimer_flow_precip,date_sel =  "2026-06-26")
...
unique(larimer_flow_precip$parameter)
```

## Load in Flow data from the CLP 
...
```{r}
stations <- get_telemetry_stations(water_district = 3, api_key = read_yaml("creds/CDWRCreds.yml")$api_key)%>%filter(meas_date_time >= "2025-08-27" & grepl("DISCHRG",parameter ))%>% filter(abbrev %nin% c("BOBGLNCO","BHERTNCO","OLGBYPCO" ))%>%
...
plot_down_days <- function(sensor_df, year_oi){
  ...
  if("user_flag" %in% names(year_data)){
    ...
    year_data <- year_data %>%
      mutate(
      #if verification status doesn't exist, make a new column where it is NA if there is no mal flag, and "OMIT" if there is a mal flag
           verification_status = ifelse(!is.na(mal_flag), "OMIT", NA))
  }
...
```
## 2026
Using sensor_data_clean we can make this plot easily

```{r}
down_day_plot_2026 <- plot_down_days(sensor_data_clean, 2026)

ggsave(here(fig_dir,"down_days_2026.png"), plot = down_day_plot_2026, width = 8, height = 12, units = "in", dpi = 300)
```
...
**This will not run until we have manually verified data **
...
date_range = range(sensor_data_clean$DT_round)


make_plot <- function(param){
  plot_data <- sensor_data_clean%>% 
    filter(parameter == param)%>%
    filter(site %in% c("bellvue", "salyer", "udall", "riverbend", "cottonwood",
                   "elc", "archery", "riverbluffs"))%>%
    mutate(DT_hourly = floor_date(DT_round, "hour"))%>%
    summarise(mean = mean(mean, na.rm = TRUE), .by = c("site", "DT_hourly", "parameter"))%>%
    left_join(labels, by = c( "parameter" = "param"))%>%
    left_join(site_names, by = "site")
  
  hist_data_plot <- hist_data_clean%>%
    filter(site %in% c("bellvue", "salyer", "udall", "riverbend", "cottonwood",
                   "elc", "archery", "riverbluffs"))%>%
    filter(parameter == param & between(plot_date, date_range[1] - days(2), date_range[2] + days(2)))%>%
    left_join(labels, by = c( "parameter" = "param"))%>%
    left_join(site_names, by = "site")%>%
    na.omit()
  
  plot_data$natural_name <- factor(plot_data$natural_name, levels = site_order)
  hist_data_plot$natural_name <- factor(hist_data_plot$natural_name, levels = site_order)
  
  param_label <-chem_expressions[[param]]
  
  if(param == "Specific Conductivity"){
    scales_param = "free_y"
  }else{
    scales_param = "free_x"
  }
  
  ggplot(hist_data_plot, aes(x = plot_date, y = median, color = natural_name))+
    geom_line(alpha = 0.7, linetype = "dashed")+
    geom_ribbon(aes(ymin = min, ymax = max), color = "grey", alpha = 0.2)+
    geom_line(data = plot_data, aes(x = DT_hourly, y = mean, color = natural_name), linewidth = 1.5, alpha = 1)+
    scale_color_manual(name = "Site", values = setNames(plot_data$site_color, plot_data$natural_name)) +
    facet_wrap(~natural_name, scales = scales_param, ncol = 2)+
    labs(x = "Date", y = param_label, color = "Site", 
         title = paste0(param, " Trends in 2026 (solid) vs Historical (2023-2025) Data"), 
         subtitle = "Dashed lines show historical daily median values, with grey ribbon showing historical daily range (max/min)")+
    theme_clean(base_size = 16)+
    theme(legend.position = "right")
}


plots <- map(unique(sensor_data_clean$parameter), make_plot)%>%
  setNames(unique(sensor_data_clean$parameter))


plots$`Specific Conductivity`
plots$DO
plots$Temperature

ggsave(plot = plots$DO, filename = here(fig_dir, "DO_Q1_hist_trends.png"), width = 16, height = 10)

ggsave(plot = plots$Temperature, filename = here(fig_dir, "Temp_Q1_hist_trends.png"), width = 16, height = 10)

ggsave(plot = plots$`Specific Conductivity`, filename = here(fig_dir, "SC_Q1_hist_trends.png"), width = 16, height = 10)
```

### WQ Event: River Bluffs Beaver Dam Failure

```{r}


dam_plot <- sensor_data_clean%>%
  filter(site == "riverbluffs" & parameter %nin% c("FDOM Fluorescence","DO", "pH", "ORP","Chl-a Fluorescence")  &
           DT_round >= ymd("2026-04-08") & DT_round <= ymd("2026-04-20"))%>%
  mutate(mean = if_else(parameter == "Turbidity" & mean > 10 & !is.na(auto_flag), NA, mean))%>%
  ggplot(., aes(DT_round, mean))+
  geom_rect(aes(xmin = as.POSIXct("2026-04-15 02:00 MST"), xmax = as.POSIXct("2026-04-15 12:00 MST"), ymin = -Inf, ymax = Inf), fill = "red", alpha = 0.005)+
  geom_line()+
  facet_wrap(~label, scales = "free")+
  labs(title = "River Bluffs Sensor Data in 2026",
       subtitle = "Note the drop in depth and increase in turbidity around 04-15 due to beaver dam failure",
       x = "Date", y = "Sensor Value")+
  theme_clean(base_size = 20)

ggsave(plot = dam_plot, filename = here(fig_dir, "River_Bluffs_Beaver_Dam_Failure.png"), width = 16, height = 10)
```
...
#WQ Event - Storm Event

```{r}
date_range = as.Date(c("2026-06-20", "2026-06-30"))

storm_plot <- function(param){
  plot_data <- sensor_data_clean%>% 
    filter(parameter == param)%>%
    filter(site %in% c("udall", "riverbend", "cottonwood",
                   "elc", "archery"))%>%
    filter(between(as.Date(DT_round), date_range[1], date_range[2]))%>%
    mutate(DT_hourly = floor_date(DT_round, "hour"))%>%
    summarise(mean = mean(mean, na.rm = TRUE), .by = c("site", "DT_hourly", "parameter"))%>%
    left_join(labels, by = c( "parameter" = "param"))%>%
    left_join(site_names, by = "site")
  ...
  ggplot(plot_data, aes(x = DT_hourly, y = mean, color = natural_name))+
    geom_line(linewidth = 1.5, alpha = 1)+
    scale_color_manual(name = "Site", values = setNames(plot_data$site_color, plot_data$natural_name)) +
    facet_wrap(~natural_name, scales = scales_param, ncol = 2)+
    labs(x = "Date", y = param_label, color = "Site", 
         title = paste0(param, ": Storm Event (6/25/2026)"))+
    theme_clean(base_size = 16)+
    theme(legend.position = "right")
}

plots <- map(unique(sensor_data_clean$parameter), storm_plot)%>%
  setNames(unique(sensor_data_clean$parameter))
plots$`Specific Conductivity`
plots$DO
plots$Turbidity

ggsave(plot = plots$DO, filename = here(fig_dir, "DO_storm_event.png"), width = 16, height = 10)
ggsave(plot = plots$Turbidity, filename = here(fig_dir, "Turb_storm_event.png"), width = 16, height = 10)
ggsave(plot = plots$`Specific Conductivity`, filename = here(fig_dir, "SC_storm_event.png"), width = 16, height = 10)

#combined plot
date_range <- as.Date(c("2026-06-20", "2026-06-30"))

params_to_plot <- c("Specific Conductivity", "DO", "Turbidity")

combined_data <- sensor_data_clean %>%
  filter(parameter %in% params_to_plot) %>%
  filter(site %in% c("salyer", "riverbend", "elc", "archery")) %>%
  filter(between(as.Date(DT_round), date_range[1], date_range[2])) %>%
  mutate(DT_hourly = floor_date(DT_round, "hour")) %>%
  summarise(mean = mean(mean, na.rm = TRUE), 
            .by = c("site", "DT_hourly", "parameter")) %>%
  left_join(labels, by = c("parameter" = "param")) %>%
  left_join(site_names, by = "site")

combined_data$natural_name <- factor(combined_data$natural_name, levels = site_order)
combined_data$parameter <- factor(combined_data$parameter, levels = params_to_plot)

param_labeller <- as_labeller(function(x) sapply(x, function(p) chem_expressions[[p]]),
                               default = label_parsed)

combined_plot <- ggplot(combined_data, aes(x = DT_hourly, y = mean, color = natural_name)) +
  geom_line(linewidth = 1.2, alpha = 1) +
  scale_color_manual(name = "Site", 
                      values = setNames(combined_data$site_color, combined_data$natural_name)) +
  facet_grid(parameter ~ natural_name, scales = "free_y", 
             switch = "y",
             labeller = labeller(parameter = param_labeller)) +
  labs(x = "Date", y = NULL, color = "Site",
       title = "Storm Event (6/25/2026)") +
...
ggsave(plot = combined_plot, filename = here(fig_dir, "storm_event_combined.png"), width = 16, height = 10)
...
## Calculate rolling averages for each year and site
...
three_year_comp <- 
  ...
  geom_line() +
  scale_color_manual(values = c("2023" =  "#745CFB", "2024" =  "#D55E00", "2025" = "#1E4D2B"))+
  scale_x_continuous(
    # Sets a tick mark every 30 days starting at 90
    breaks = seq(90, 320, by = 30), 
    # Converts DOY to "Month Day" format
  ...
  facet_grid(label ~ natural_name, scales = "free_y") +
  coord_cartesian(xlim = c(90, 320)) # Better practice than xlim() as it doesn't remove data points

ggsave(here(fig_dir, "do_ph_sc_temp_2023_2025_comp.png"),plot = three_year_comp, width = 16, height = 16, units = "in", dpi = 300)
...
# Current WY flow analysis vs historic min/max

```{r}
library(cdssr)
library(jsonlite)

#using CDWR lincoln, boxelder, and fossil creek gauges

gauges <- c("CLAFORCO", "CLABOXCO", "CLARIVCO")

gauge_labels <- c(
  CLAFORCO = "Lincoln",
  CLABOXCO = "Boxelder",
  CLARIVCO = "Poudre River Below Fossil Creek")

wy_start   <- as.Date("2025-10-01")
today_date <- Sys.Date()

# Pull daily discharge from CDWR for a station over a date range
get_dwr_daily <- function(abbrev, start_date, end_date){
  url <- paste0(
  ...
  resp <- GET(url)
  dat <- fromJSON(content(resp, "text", encoding = "UTF-8"))$ResultList
  ...
  dat %>%
    mutate(
      abbrev = abbrev,
      measDate = as.Date(measDate)
    )
}

#historic data
hist_raw <- map_df(gauges, ~get_telemetry_ts(abbrev = .x, 
                                             parameter = "DISCHRG", 
                                             start_date = ("2000-01-01"), 
                                             end_date = format(today_date, "%Y-%m-%d"),
                                             timescale = "day"))
hist_raw <- hist_raw %>%
  mutate(
    abbrev = factor(abbrev, levels = gauges),
    water_year = if_else(month(meas_date) >= 10, year(meas_date) + 1, year(meas_date)),
    plot_year  = if_else(month(meas_date) >= 10, 2025, 2026),
    plot_date  = as.Date(paste0(plot_year, "-", month(meas_date), "-", day(meas_date)),
                          format = "%Y-%m-%d")
  ) %>%
  filter(!is.na(plot_date))

hist_stats <- hist_raw %>%
  filter(water_year != 2026) %>%
  summarise(
    median = median(.data$meas_value, na.rm = TRUE),
    min    = min(.data$meas_value, na.rm = TRUE),
    max    = max(.data$meas_value, na.rm = TRUE),
    .by = c("abbrev", "plot_date")
  )

#current wy data
current_wy <- hist_raw %>%
  filter(water_year == 2026) %>%
  select(abbrev, meas_date, plot_date, meas_value)

today_plot_date <- current_wy %>% pull(plot_date) %>% max(na.rm = TRUE)

WY_flow_plot <- ggplot(hist_stats, aes(x = plot_date, y = median, color = abbrev)) +
  geom_line(alpha = 0.7, linetype = "dashed") +
  geom_ribbon(aes(ymin = min, ymax = max), color = "grey", alpha = 0.2) +
  geom_line(data = current_wy, aes(x = plot_date, y = meas_value, color = abbrev),
            linewidth = 1.5, alpha = 1) +
  scale_x_date(limits = c(wy_start, today_plot_date), date_labels = "%b") +
  scale_y_log10()+
  facet_wrap(~abbrev, scales = "free_y", ncol = 1,
             labeller = labeller(abbrev = gauge_labels)) +
  labs(
    x = "Date", y = "Discharge (cfs)", color = "Gauge",
    title = "2026 WY Flow vs Historical Min/Max",
    subtitle = "Dashed line = historical daily median; grey ribbon = historical daily min/max range"
  ) +
  theme_clean(base_size = 15) +
  theme(legend.position = "none",
...
ggsave(plot = WY_flow_plot, filename = here(fig_dir, "WY_flow_plot.png"), width = 16, height = 10)
