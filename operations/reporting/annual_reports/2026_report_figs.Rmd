...
combined_data <- sensor_data_clean %>%
  ...
  filter(site %in% c("udall", "riverbend", "cottonwood", "elc", "archery")) %>%
  filter(between(as.Date(DT_round), date_range[1], date_range[2])) %>%
  mutate(DT_hourly = floor_date(DT_round, "hour")) %>%
...
gauges <- c("CLARIVCO", "CLAFORCO", "CLABOXCO")

gauge_labels <- c(
  CLAFORCO = "Lincoln",
  CLABOXCO = "Boxelder",
  CLARIVCO = "Fossil Creek")
...
#historic data
hist_raw <- map_df(gauges, ~get_dwr_daily(.x, as.Date("2000-01-01"), today_date))
hist_raw <- hist_raw %>%
  mutate(
    water_year = if_else(month(measDate) >= 10, year(measDate) + 1, year(measDate)),
    plot_year  = if_else(month(measDate) >= 10, 2025, 2026),
    plot_date  = as.Date(paste0(plot_year, "-", month(measDate), "-", day(measDate)),
                          format = "%Y-%m-%d")
  ) %>%
...
hist_stats <- hist_raw %>%
  ...
  summarise(
    median = median(.data$measValue, na.rm = TRUE),
    min    = min(.data$measValue, na.rm = TRUE),
    max    = max(.data$measValue, na.rm = TRUE),
...
current_wy <- hist_raw %>%
  filter(water_year == 2026) %>%
  select(abbrev, measDate, plot_date, measValue)
...
WY_flow_plot <- ggplot(hist_stats, aes(x = plot_date, y = median, color = abbrev)) +
  geom_line(alpha = 0.7, linetype = "dashed") +
  geom_ribbon(aes(ymin = min, ymax = max), color = "grey", alpha = 0.2) +
  geom_line(data = current_wy, aes(x = plot_date, y = measValue, color = abbrev),
