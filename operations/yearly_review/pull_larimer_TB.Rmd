...
```{r}
station_ids <- read_csv("data/metadata/larimer_county_station_data.csv", show_col_types = F)%>%
...
for(i in 1:length(station_ids)){
  ...
  print(paste0("On station ", i, " of ", length(station_ids), " with id ", station_ids[i], " and n rows = ", nrow(import)))
  if(nrow(import) == 0){
    q_site <- tibble()
  }else{
  q_site <- import%>%
...
meta <- read_csv("data/metadata/larimer_county_station_data.csv", show_col_types = F) %>%
...
#Save data
#write_csv(larimer_precip, 'data/metadata/larimer_county_station_precip_2025.csv')
...
larimer_precip <- read_csv('data/metadata/larimer_county_station_precip_2025.csv', show_col_types = F)%>%
...
#aug_storm <- 
  ggplotly(create_precip_plot(start_date = as.Date("2025-07-25"), end_date = as.Date("2025-09-06"), grouping = "ws"))
...
create_precip_map("2025-08-02")
dates <- seq(as.Date("2025-07-01"), as.Date("2025-09-01"), by = "day")
