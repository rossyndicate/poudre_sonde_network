...
create_precip_plot <- function(start_date, end_date, grouping){
  ...
  if(grouping == "ws"){
    ...
    summarise(
      total_precip = sum(p_in),
      num_sites = n_distinct(site),
      avg_daily_precip = total_precip / num_sites,
      .groups = "drop")
    
    #make a histogram bar plot with dt mst as X axis and color by name
ggplot(precip_trim, aes(x = date,y = avg_daily_precip, fill = watershed))+
  geom_col()+
 ...
 # geom_rect(aes(xmin = date - 0.5, xmax = date + 0.5, ymin = 0, ymax = max(avg_daily_precip)), fill = NA, color = "black")+
  labs(title = 'Larimer County Precipitation',
       x = 'Date',
       y = 'Precip (WS daily precip/sites)')+
  theme_bw()
...
ggplotly(create_precip_plot(start_date = as.Date("2024-05-01"), end_date = as.Date("2024-08-29"), grouping = "site"))
...
create_precip_map <- function(date_sel){
  ...
  date_sel = as.Date(date_sel)
    ...
    filtered_data <- larimer_precip  %>%
      ...
      summarise(daily_precip = sum(p_in), .groups = "drop")%>%
      st_as_sf(coords = c('long', "lat"), crs = 4326)
...
at_10 = c(0.01, 0.05, 0.1, 0.2,  0.25, 0.5, 0.75, 1, 1.25, 1.5, 2)
      ...
      if(nrow(non_zero_data) > 0){
        map <- mapview::mapview(non_zero_data,
          zcol = "daily_precip",
          layer.name = paste0("Daily Precipitation on ", date_sel),
          col.regions = palfunc, 
          at = at_10)
      }
      if(nrow(non_zero_data) == 0){
