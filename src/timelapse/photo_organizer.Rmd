...
sites <- c("tamasag", "legacy", "timberline", "prospect", "boxelder", "archery", "riverbluffs")

for(i in 1:length(sites)){
  
  site <- sites[i]
  dir.create(file.path(paste0("data/timelapse_photos/2023_compiled/", site)))
  
  photo_folders <- list.files(path = "data/timelapse_photos", pattern = paste0(site, "_2023"), full.names = TRUE)%>%
  ...
  map(photo_folders, compile_files)
}
