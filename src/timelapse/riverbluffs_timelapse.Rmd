---
title: "River Bluffs Timelapse Video"
author: "Katie Willi"
date: "12/29/2022"
...
## Photo timestamps
```{r}
photo_files_1 = list.files('data/timelapse_photos/riverbluffs_20231011_20231015', full.names = T)
photo_files_2 = list.files('data/timelapse_photos/riverbluffs_20231015_20231105', full.names = T)


photo_time = read_exif(photo_files_1) %>%
...
photo_time_2 = read_exif(photo_files_2) %>%
  # Files are named with their hour-rounded timestamp
  arrange(FileName) %>%
  mutate(DT_round = round_date(ymd_hms(str_sub(FileModifyDate, 1, 19), tz = "MST"), "30 minutes")) %>%
  select(SourceFile, DT_round)
...
wq_data <- list.files(path = "data/api/historical_api_data/", full.names = TRUE, pattern = "River Bluffs") %>%
  map_dfr(~read_csv(.)) %>%
  #map_dfr(~data.table::fread(.) %>% select(-id)) %>%
  # remove overlapping API-pull data
  distinct() %>%
  filter(ymd_hms(timestamp) >= "2023-10-01 10:00:00" & ymd_hms(timestamp) <= "2023-11-30 08:30:00") %>%
  filter(parameter %in% c("Depth", "Chl-a Fluorescence")) %>%
  select(timestamp, parameter, value) %>%
  pivot_wider(names_from = "parameter", values_from = "value")

photo_join <- inner_join(photo_time, wq_data, by= c("DT_round"="timestamp")) %>%
  arrange(DT_round) %>%
  #.[1:260,] %>%
  rowid_to_column()
```
