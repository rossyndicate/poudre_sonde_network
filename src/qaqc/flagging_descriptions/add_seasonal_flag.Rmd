...
add_seasonal_flag <- function(df) {
  ...
  df <- df %>%
    ...
    add_flag((mean < t_mean01 | mean > t_mean99), paste("outside of", parameter_name, "seasonal range")) %>%
...
add_flag((mean < t_mean01 | mean > t_mean99), 
         paste("outside of", parameter_name, "seasonal range"))
```
...
- `paste("outside of", parameter_name, "seasonal range")` = Dynamic flag descriptor inserted into the `flag` column.
