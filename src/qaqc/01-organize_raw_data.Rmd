...
# result <- imap(all_data_flagged, ~generate_histograms(.x, .y))
...
temperature_hist_plots <- imap(temperature_hist_list, ~generate_histograms(.x, .y))
...
```{r}
# Working on seasonal histograms
winter <- c(12,1,2)
spring <- c(3,4,5)
summer <- c(6,7,8)
fall <- c(9,10,11)
...
for (i in colnames(seasons)){
  ...
  df <- all_data_flagged[["archery-Temperature"]] %>%
    filter(month %in% seasons[[i]]) 
  
  if (all(is.finite(df$mean))) {
