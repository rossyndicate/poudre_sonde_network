...
```{r}
manual_flag_removal <- function(df_from_list) {
  # Generating df name to pull from all_data_flagged list
  site_name <- str_extract(deparse(substitute(df_from_list)), ".*(?= - )")
  parameter_name <- str_extract(deparse(substitute(df_from_list)), ".*(?<= - )")
  site_param <- paste0(site_arg, "-", parameter_arg)
  
  daily_flag_list <- generate_daily_flag_plots(site_arg = site_name, 
                                               parameter_arg = parameter_name, 
                                               flag_arg = NULL)
  
  
  
  # make plot_data_list and bind it together
  plot_data_list
  for (i in daily_flag_list) {
...
display_plots <- function(plot_object) {
  ...
  for (i in 1:nrow(flag_data)){
    ...
    choice <- get_flag_decision("Do you want to continue?")
      if (choice) {
        cat("You chose to continue.\n")
      } else {
        cat("You chose to exit.\n")
      }
