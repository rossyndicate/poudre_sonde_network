...
# Lastly, we swapped Boxelder's sonde out for Rist's late in 2022:
  mutate(site = case_when(site == "rist" & DT > "2022-09-20" & DT < "2023-01-01" ~ "boxelder"))
``` 
...
- Timberline was buried starting "2022-04-22 00:00:00" and its RDO cap was in really poor condition at the beginning of 2022 field season until "2022-05-25 16:00"
- Explore back calibrating all turbidity data - it is recommended not to calibrate. Does calibrating actually make turbidity worse?
...
raw_temperature <- all_data %>%
  ...
  mutate(flag = ifelse((p1 <= rollavg - (3 * rollsd) | p1 >= rollavg + (3 * rollsd)), "Outside SD range", NA))
```
