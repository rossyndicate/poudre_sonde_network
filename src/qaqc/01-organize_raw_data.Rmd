...
# this really does not need to be a yaml solution
...
```{r}
# Adding flags related to slopes between points
# Flag when the slope between a data point and its previous or next point exceeds the 
# .99 quantile for slopes.
...
add_large_anomaly_flag <- function(df) {
  ...
  df <- df %>% 
    ...
    mutate(flag_binary = ifelse((is.na(flag) | str_detect(x, "^(24hr anomaly|anomaly window|24hr anomaly;\\nanomaly window)$")), 0, 1),
           roll_bin = data.table::frollsum(flag_binary, n = 97, align = 'center', na.rm = F, fill = NA_real_)) %>% 
    add_flag((roll_bin >= (97*0.25)), "24hr anomaly")
