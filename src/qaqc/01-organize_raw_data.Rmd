...
# all_data_summary_stats_list[["archery-Temperature"]] %>% add_field_flags()
# all_data_summary_stats_list[["archery-Temperature"]] %>% add_range_flags()
# all_data_summary_stats_list[["archery-Temperature"]] %>% add_slope_flags()
...
add_all_flags <- function(df) {
  ...
  df <- df %>%
    ...
    mutate(flag_binary = ifelse(is.na(flag), 0, 1),
           roll_bin = data.table::frollsum(flag_binary, n = 97, align = 'center', na.rm = F, fill = NA_real_)) %>%
    # is this supposed to flag the entire 24 hour period?
    # This should flag it and also make sure to check that it is not taking into account the same flag when it is doing it again.
...
# test <- generate_daily_flag_plots("archery", "Actual Conductivity", "outside of Actual Conductivity seasonal range")
# test
# map(params, ~generate_daily_flag_plots("archery",.x, "slope flag"))
# surrounding weird is really inclusive...
# map(sites, ~generate_daily_flag_plots(.x, "Chl-a Fluorescence", NULL))


# Capture 62 days of suspicious data, which is more than we did before,
# but all things considered not a super crazy high number either
```
