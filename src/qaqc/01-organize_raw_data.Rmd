...
generate_summary_statistics <- function(site_param_df) {
  ...
  summary_stats_df <- site_param_df %>%
    ...
    mutate(
      ...
      m_mean90 = quantile(mean, 0.99, na.rm = TRUE),
...
add_field_flags <- function(df) {
  ...
  # adding the site visit window flag
  # This is not working properly, will need to go back to this
  df <- tryCatch({
    df <- df %>% 
      add_flag(lag(grepl("site visit", flag, fixed = T), n = 3, default = 1), "site visit window")
    },
    error = function(err) {
        df})
...
# testing ----
df <- all_data_summary_stats_list[["prospect-Actual Conductivity"]]
df %>% add_field_flags %>% View
```
...
add_range_flags <- function(df) {
  ...
  df <- df %>%
    ...
    add_flag(parameter == parameter_name & (mean < m_mean10 | mean > m_mean90),
...
add_all_flags <- function(df) {
  ...
  df <- df %>%
    ...
    add_flag((roll_bin >= (97*0.25)), "Surrounding weird")
}
...
generate_daily_flag_plots <- function(site_arg, parameter_arg, flag_arg = NULL) {
  ...
  for (i in 1:nrow(site_flag_dates)) {
    ...
    plot <- ggplot(data = plot_data) +
      ...
      theme_bw() +
      theme(legend.position = 'bottom') +
      ggtitle(paste("Flags at", site_arg, "on", as.character(flag_day))) + # HERE
      labs(x = "Datetime",
           y = "Mean")
...
# This is still not getting all of the data that I am expecting -j
generate_daily_flag_plots("archery", "Temperature", "slope flag suspect")
```
