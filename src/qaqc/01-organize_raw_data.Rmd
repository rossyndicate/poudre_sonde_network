...
```{r}
# This will need to get done for each site-param combo separately
...
sample_temp_stats <- temperature_archery_summary_stats %>% 
  filter(year(DT_round) == 2022,
         month == 5,
         day(DT_round) >= 10 & day(DT_round) <= 10)
        # hour(DT_round) >= 4 & hour(DT_round) <= 5)
...
View(add_sensor_spec_range_flags(archery_objects_df))
```


```{r}
# sensor spec ranges flagging function
# this should be done after the dfs for every site are joined ?
...
temperature_archery_summary_stats_2 <- temperature_archery_summary_stats_2 %>% 
...
# Current vis:
ggplot() +
  geom_point(data = filter(temperature_archery_summary_stats_2, is.na(flag)), 
...
flag_dates <- temperature_archery_summary_stats_2 %>% 
  filter(str_detect(flag, "Slope exceedance")) %>%  
...
for (i in 1:nrow(flag_dates)) {
  ...
  day_data <- temperature_archery_summary_stats_2 %>%
  ...
  plot <- ggplot(data = day_data, aes(x=DT_round, y = mean, color = flag)) +
    ...
    geom_vline(data = (day_data %>% filter(is.na(mean))), aes(xintercept = DT_round, color = "NA mean")) +
...
```{r}
temperature_archery_summary_stats_3 <- temperature_archery_summary_stats_2 %>% 
  add_flag(slope_behind >= slope_thresh, "slope (behind)")
```

```{r}
# Visualizing slope exceedance (behind)
plot_list <- list()

flag_dates_2 <- temperature_archery_summary_stats_3 %>% 
  filter(str_detect(flag, "slope (behind)"))#%>%   
  # group_by(day(DT_join)) %>% 
  # slice(1)

for (i in 1:nrow(flag_dates_2)) {
  
  flag_year <- flag_dates_2$year[i]
  flag_month <- flag_dates_2$month[i]
  flag_day <- flag_dates_2$DT_round[i]
  
  day_data <- temperature_archery_summary_stats_3 %>%
    filter(year == flag_year,
           month == flag_month,
           day(DT_round) == day(flag_day))
