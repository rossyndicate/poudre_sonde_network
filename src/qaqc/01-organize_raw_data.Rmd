...
field_notes <- read_excel("data/sensor_field_notes.xlsx") %>%
  ...
  arrange(DT_round) %>%
  # binary version of deployed/pulled options. may need to play around with this som
  # more
...
Filter instances in which a sonde was pulled out of field mid-season (basically a backup
filter in case a sonde was pulled out of water but the log didn't get stopped)
...
temperature_archery <- all_data %>%
  ...
  pad(by = "DT_round") %>%
  full_join(filter(dplyr::select(field_notes, where, DT_round, site), site == "archery"), 
            by = c('DT_round'))

temperature_archery <- cbind(select(temperature_archery, -where), data.table::setnafill(x = temperature_archery[,5], type = "locf")) %>%
  # add flag for no data times
  mutate(flag = if_else(is.na(p1), 'no data', NA_character_)) %>% 
  ...
  mutate(slope_ahead = abs(front1 - p1)/15,
         ...
         quarter = quarter(DT_round),
         year = year(DT_round),
         y_q = paste(year, '-', quarter))
...
For wonky data next to each other (defined by roll sd) - here we make some seasonal
summaries to help with this

```{r}
temperature_summary <- temperature_archery %>% 
  group_by(y_q) %>% 
  summarise(yq_sd = sd(p1, na.rm = T))
```
...
temperature_archery <- temperature_archery %>%
  ...
  mutate(flag = if_else((p1 <= rollavg - (3 * yq_sd) | p1 >= rollavg + (3 * yq_sd)),
                        "Outside SD range", 
                        flag))
