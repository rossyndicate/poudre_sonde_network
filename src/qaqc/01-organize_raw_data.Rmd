...
add_range_flags <- function(df) {
  ...
  sensor_min <- eval(parse(text = parameter_ranges[[parameter_name]]$sensor_specifications$min))
  sensor_max <- eval(parse(text = parameter_ranges[[parameter_name]]$sensor_specifications$max))
...
```{r}
# Working on seasonal histograms function 
# useful for seeing seasonal differences and how each season affects the 
# general histogram

generate_seasonal_histogram <- function(df) {
  ...
  for (i in colnames(seasons)){
    ...
    # This if else is to check for data availability, could be a try() in the future
    if (all(is.finite(filtered_df$mean))) {
...
generate_seasonal_histogram(df=all_data_flagged[["archery-Temperature"]])
```
