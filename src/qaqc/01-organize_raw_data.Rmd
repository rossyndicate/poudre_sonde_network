...
# testing ----
df <- all_data_summary_stats_list[["archery-Temperature"]]
...
generate_daily_flag_plots <- function(site_arg, parameter_arg, flag_arg = NULL) {
  ...
  site_flag_dates <- all_data_flagged[[site_param]] %>%
      ...
      # why does adding the year here not exclude the 2023 data, but without it the 2023 data is excluded?
  ...
  for (i in 1:nrow(site_flag_dates)) {
    ...
    plot <- ggplot(data = plot_data) +
      ...
      theme_bw() +
      theme(legend.position = 'bottom') +
      ggtitle(paste(flag_arg ,"at", site_arg, "on", as.character(flag_day))) + # HERE
      labs(x = "Datetime",
           y = "Mean")
