...
n_unique_dates <- length(unique(as.Date(all_data_summary_df$DT_round)))
```
...
add_slope_flags <- function(df) {
  ...
  df <- df %>%
    # adding slope flag
    add_flag((slope_ahead >= m_slope_behind_99 | slope_behind >= m_slope_behind_99), "slope flag suspect")
...
add_large_anomaly_flag <- function(df) {
  ...
  for (i in 1:48) {
   df <- df %>% 
     add_flag((is.na(flag) | !str_detect(flag, "anomaly window") & lag(str_detect(flag, "24hr anomaly flag"), n = i)), "anomaly window") %>% 
     add_flag((is.na(flag) | !str_detect(flag, "anomaly window") & lead(str_detect(flag, "24hr anomaly flag"), n = i)), "anomaly window")
  }
  return(df)
}
```
...
# map(params, ~generate_daily_flag_plots("archery", .x, "outside SD range"))
test <- generate_daily_flag_plots("prospect", "Temperature", "missing data")
head(test, n = 20)
length(test)
...
length(generate_weekly_flag_plots("archery", "Temperature", "slope flag suspect"))
```
...
tail(weekly_daily_flag_plots("archery", "DO", "seasonal range"))
```
...
head(imap(all_data_flagged, ~generate_seasonal_histogram(.x, .y)))
