...
#... or, manual selection of start time
#start <- Sys.time() - lubridate::days(2000)  #Sys.time() - lubridate::days(7)
end <- Sys.time()
sites <- c("Rist", 
...
api_puller <- function(site, 
  ...
  # start <- list.files('data/api/', pattern = site, full.names = FALSE) %>%
  #   str_sub(., -14, -5) %>%
  #   max(.)
  # 
...
data <- sites %>% map_dfr(~api_puller(site = .,
                                      start = start,
                                      end = end)) #%>%
bind_rows()
```
