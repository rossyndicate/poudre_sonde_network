...
first_pass <- function(what, parama, max=100000, min=-100000){
  ...
  # rollmeaner <- function (x, aligner = "center") {
  #   if (length(x) < 5) {
  #     rep(NA,length(x)) 
  #   } else {
  #     rollmean(x, 2, align = aligner, na.pad = TRUE, na.rm = T)
  #       
  #   }
  # }
  # 
  # rollsder <- function (x, aligner = "center") {
  #   if (length(x) < 5) {
  #     rep(NA,length(x)) 
  #   } else {
  #     rollapplyr(x, 5, align = aligner, FUN = sd, fill = NA) 
  #   }
  # }
  ...
  it <- what %>%
    ...
    # # mutate(roll_up = lag(rollmeaner(p1, aligner = "left"), n=2),
    # #        roll_down = lead(rollmeaner(p1, aligner = "right"), n=2)) %>%
    # mutate(#rollavg1 = (roll_up+roll_down)/2,
    #   front1=(lead(p1, n=1)),
    #   front2=(lead(p1, n=2)), 
    #   back1=(lag(p1, n=1)), 
    #   back2=(lag(p1, n=2))) %>%
    # group_by(DT) %>%
    # mutate(rollsd = sd(unlist(select(cur_data(), front1:back2))),
    #        rollavg=mean(unlist(select(cur_data(), front1:back2)))) %>%
    # ungroup() %>%
    # mutate(sd3 = ifelse((p1 <= rollavg-(3*rollsd) | p1 >= rollavg+(3*rollsd)), 1, 0)) 
...
timberline_depth <- timberline_depth %>%
  mutate(Depth_ft_p1 = ifelse(DT > '2022-09-14 13:15:00', 
                              Depth_ft_p1 +
                                abs(
                                  filter(dplyr::select(rawless_timberline, c(DT, Depth_ft)), DT == ymd_hms('2022-09-14 13:00:00'))$Depth_ft_p1 -
                                    filter(dplyr::select(rawless_timberline, c(DT, Depth_ft)), DT == ymd_hms('2022-09-14 13:30:00'))$Depth_ft_p1
                                ), 
                              Depth_ft_p1)) %>%
...
```{r}
# all_rawless <- bind_rows(rawless_rist,rawless_legacy,rawless_lincoln,rawless_timberline,rawless_prospect,rawless_elc,rawless_archery) %>%
#   mutate(Chl_a= ifelse(is.na(Chla), Chla_RFU, NA),
#          Chl_a = ifelse(is.na(Chl_a), Chla_µg_L/10, NA))
```
...
# 
# do <- all_rawless %>%
#   filter(DO_ppm > 0 & DO_ppm < 25) %>%
#   filter(Depth_ft > 0 & Depth_ft < 10) %>%
#   filter(Water_Temp_C > 0 & Water_Temp_C < 26) %>%
#   #filter(!is.na(DT)) %>%
#   mutate(daily = as_date(ymd_hms(DT))) %>%
#   mutate(hourly = ymd_hms(as.character(round_date(ymd_hms(DT), "1 hour")))) %>%
#   #group_by(site,daily, hourly) #%>%
#   # summarize(Water_Temp_C = mean(Water_Temp_C, na.rm=TRUE),
#   #          pH1 = mean(pH1, na.rm=TRUE),
#   #          ORP_mV = mean(ORP_mV, na.rm=TRUE),
#   #          Specific_Conductivity_µS_cm = mean(Specific_Conductivity_µS_cm, na.rm=TRUE),
#   #          DO_ppm = mean(DO_ppm, na.rm=TRUE),
#   #          Turbidity_NTU = mean(Turbidity_NTU, na.rm=TRUE),
#   #          Chl_a = mean(Chl_a, na.rm=TRUE),
#   #          Depth_ft = mean(Depth_ft, na.rm=TRUE)) %>%
#   mutate(month = month(hourly),
#          year = year(hourly))
# 
# october_pulse <- do %>% filter(as_date(daily) > ymd('2022-10-18') & as_date(daily) < ymd('2022-10-26')) %>%
#   select(DT,site,DO_ppm,Specific_Conductivity_µS_cm,Depth_ft)%>%
#   pivot_longer(cols=-c(DT,site), names_to="Parameter")
# 
# 
# bw_event <- do %>% filter(as_date(daily) > ymd('2022-08-15') & as_date(daily) < ymd('2022-08-23')) %>%
#   select(DT,site,DO_ppm,Specific_Conductivity_µS_cm,Depth_ft)%>%
#   pivot_longer(cols=-c(DT,site), names_to="Parameter")
# 
# runoff <- do %>% filter(as_date(daily) > ymd('2022-05-28') & as_date(daily) < ymd('2022-06-07')) %>%
#   select(DT,site,DO_ppm,Specific_Conductivity_µS_cm,Depth_ft)%>%
#   pivot_longer(cols=-c(DT,site), names_to="Parameter")
# 
# 
# ggplot(runoff)+
#   geom_path(aes(x = DT, y = value, color = site)) +
