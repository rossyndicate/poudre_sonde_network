...
drift_corrected_2025 <- map_dfr(drift_corrected_2025_path, read_parquet) %>%
  data.table() %>%
  fcw.qaqc::standardize_site_names() %>%
...
threshold_check <- map_dfr(
  ...
  ~{
    .x %>%
      ...
      summarize(
        row_min_check = n() >= 7000,
        val_min_check = sum(!is.na(mean)) >= 5000,
        pct_min_check = (sum(!is.na(mean)) / n()) >= 0.70,
        passes_all = row_min_check & pct_min_check & val_min_check,
        .groups = "drop"
      )
  }
) %>%
...
group_by(parameter, season) %>% 
  mutate(
    t_slope_behind_01 = ifelse(is.na(t_slope_behind_01), min(t_slope_behind_01, na.rm = TRUE), t_slope_behind_01),
    t_slope_behind_99 = ifelse(is.na(t_slope_behind_99), max(t_slope_behind_99, na.rm = T), t_slope_behind_99)
  )%>%
  ungroup()%>%
  mutate(
  ...
  #if t_mean_01 is less than the realistic threshold, replace with the realistic threshold. If t_mean_99 is greater than the realistic threshold, replace with the realistic threshold.
  mutate(
    t_mean01 = ifelse(t_mean01 < min, min, t_mean01),
    t_mean99 = ifelse(t_mean99 > max, max, t_mean99)
  ) %>%
