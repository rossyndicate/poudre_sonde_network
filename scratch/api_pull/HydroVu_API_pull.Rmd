...
library('tidyverse')
library('HydroVuR')
source('api_pull/hv_getdata_id.R')
source('api_pull/hv_locations_all.R')
```
...
```{r}
creds = yaml::read_yaml('api_pull/credentials.yml')
...
```{r}
write.csv(one_df, file.path(dump_dir, '2023-06_apipull.csv'), row.names = F)
```
