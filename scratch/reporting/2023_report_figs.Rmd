...
bg_colors <- c("#01377D", "#009DD1", "#97E7F5", "#7ED348", "#26B170")
...
plotter <- function(clean = FALSE, hourly = TRUE, sites = c("legacy", "timberline" ,"boxelder",  "archery", "river bluffs"),
  ...
  grab_site_data <- function(site){
   ...
   flag_plot_data <- flag_plot_data %>%
     filter(is.na(cleaner_flag))
    # filter(!grepl("suspect data", flag))%>%
    # filter(!grepl("outside of seasonal range", flag))%>%
    # filter(!grepl("sv window", flag))%>%
    # filter(!grepl("slope violation", flag))
  ...
  all_flag_plot_data$site <- factor(all_flag_plot_data$site, levels = sites)
  
  ggplot() +
    geom_line(data = all_flag_plot_data, 
               aes(x = DT_round, y = mean, color = site), size = 2) +
...
clean_temp_may <- plotter(sites = may_sites, clean = TRUE, parameter = "Temperature", units = "Temperature (C)")+
  ...
  ggsave("data/sharing/figures/2023/DRAFT_BG_urban_storm_may_2023.jpg",width = 20, height = 14, dpi = 300)
```
...
july_sites <- c("tamasag","timberline", "prospect", "archery")
...
horse_start_date <- "2023-10-15 12:00:00"
horse_end_date <- "2023-11-02 12:00:00"
...
ggsave("data/sharing/figures/2023/DRAFT_Red_horsetooth_release_oct.jpg",width = 20, height = 14, dpi = 300)
