...
# Budget update 9/25/23

```{r}


budget_202309 <- read_csv("data/sharing/5303303_Budget_20230925.csv")%>%
  mutate(date = as_date(DATE, format = "%m/%d/%y"), 
         year = year(date))%>%
  filter(year == 2023)

allotments <- budget_202309%>%
  dplyr::rename(person = `VENDOR PERSON`)%>%
  filter(person == "ALLOTMENT")

cost_by_user <- budget_202309%>%
  dplyr::rename(person = `VENDOR PERSON`)%>%
  filter(person != "ALLOTMENT")%>%
  group_by(person)%>%
  summarise(cost =sum(EXPENSE))%>%
