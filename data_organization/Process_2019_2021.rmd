...
```{r}
old_raw <- read_csv("data/sensor_data/2018_2021/kw_collation/all_sensor_data_pre2022.csv") %>%
...
raw_data_standardized <- old_raw %>%
  pivot_longer(-c(DT, data, year, date, sensor, comments), values_to = "value", names_to = "var") %>%
  ...
  rename(site = sensor,
         timestamp = DT) %>%
  mutate(parameter = case_when(var == "Water_Temp_C" ~ "Temperature",
                               var == "pH" ~ "pH",
                               var == "ORP_mV" ~ "ORP",
