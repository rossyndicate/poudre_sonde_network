...
cal_range_values <- function(sensor_df_arg, end_dt,
    ...
    if (length(matching_networks) > 0) {
      ...
      if (length(last4_dts) > 0) {
        ...
        if (length(neighbor_vals) > 0) {
          to_val_c <- mean(neighbor_vals, na.rm = TRUE)
        }
      }
    }
  }
  ...
  if (length(passing) > 0) {
    best <- passing[[which.max(sapply(passing, function(x) abs(x$from - x$to)))]]
    return(c(best$from, best$to))
  }

  # Fall back: mix best finite endpoints from A
  if (a_valid) return(c(from_val_a, to_val_a))
...
```{r}
prepped_data <- prepped_data[order(
    !str_detect(names(prepped_data), "Turbidity"),
    names(prepped_data)
  )]
```

```{r}
plot_drift(prepped_data$`pbd-Turbidity`, prepped_data, site_order)
```
...
After all of this work, the final column to consider is: `mean_smoothed`
...
plot_final_corrections(
  df                  = corrected_data$`elc-Turbidity`,
  data_list           = prepped_data,
  site_order_template = site_order
)
```
