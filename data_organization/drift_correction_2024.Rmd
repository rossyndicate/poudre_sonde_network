...
#' @param drift_type "linear" (default), "exponential", "log", "uniform", or "fitted_linear"
...
#'   For "fitted_linear", an OLS line is fit to the observed values in the window.
#'   The slope b is used in a hyperbolic multiplicative correction anchored at
#'   post_val: corrected(t) = observed(t) * post_val / (post_val + b * wt(t)).
...
plot_drift <- function(df, data_list, site_order_template) {
  ...
  site_name_lookup <- stringr::str_remove(site_name, "_virridy$")
  matching_networks <- keep(site_order_template, ~ site_name_lookup %in% .x)

  above_df <- NULL
  below_df <- NULL

  if (length(matching_networks) > 0) {
    if (site_name_lookup %in% names(matching_networks)) {
      sites_order <- matching_networks[[site_name_lookup]]
    } else {
...
#'   boundary values that bracket each drift correction window. Two independent
...
#'
#'   If both options pass all checks, the one with the larger `|from - to|` is
#'   returned because it represents the strongest available evidence of drift.
#'   If differences are equal (including both zero), Option A is preferred.
#'   If only one option passes, that option is returned.
#'   If Option A is incomplete (one or both endpoints are non-finite), the
#'   function falls back to mixing the best finite endpoint from each option
#'   individually — this preserves the original single-source behavior and
#'   prevents a tiny field-note drift from collapsing `exp_k`'s denominator.
