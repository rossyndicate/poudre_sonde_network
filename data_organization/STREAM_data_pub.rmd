...
all_sites %>% 
  ...
  map(function(site_polygon) {
    ...
    mapview::mapview(site_polygon, layer.name = paste("Catchment:", current_id)) + 
    mapview::mapview(site_point, layer.name = "Pour Point", col.regions = "red")
  })
...
ross_metadata_final <- all_sites %>%
  ...
  filter(!Station_ID %in% c("springcreek", "boxcreek")) #removing these two sites since they are not in the STREAM data request
...
all_sensor_data <- read_parquet(input_all_sensor_data_path)%>%
  filter(site %nin% c("pman_fc", "pbr_fc", "mtncampus", "springcreek", "boxcreek"))%>%
...
all_sensor_data_clean <- all_sensor_data %>%
  mutate(final_value = mean_cleaned, 
         ...
         final_status = case_when(status == "auto_flagged" & !is.na(final_value) ~ "P",
                                  status == "auto_flagged" & is.na(final_value) ~ "R",
                                  is.na(final_value) ~ "R",
                                  status == "in_progress_verified" & !is.na(final_value) & is.na(clean_flag) ~ "A",
                                  status == "in_progress_verified" & !is.na(final_value)& !is.na(clean_flag) ~ "P"))%>%
