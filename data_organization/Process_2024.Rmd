...
process. The final flagged dataset will be saved for manual review and 
verification.
...
write_rds(calibrated_data, here("data", "raw", "sensor", "manual_data_verification", "2024_cycle","hydro_vu_pull","back_calibration", "calibrated_sensor_data_2024.rds"))
...
```{r}
calibrated_data_final <- read_rds("data/raw/sensor/manual_data_verification/complete_dataset/calibrated_sensor_field_data_finalized.rds")[["2024"]] # Only extract the 2024 data
...
site_order_2024 <- load_site_order( here("data","derived","auto_qaqc_files", "site_order", "site_order_2024.yml"))
...
This tool is located in the script `data_organization/drift_correction_2024.Rmd`. Here we are applying the drift corrections identified in the manual verification tool to the data and then saving the output in the "post_verification" folder.

## Read in pre-calibration raw values

The 2024 data was processed before the `mean_pre_cal` column was added to the pipeline. Raw pre-calibration values are recovered here from `raw_data.parquet` and joined to the verified data downstream.
...
The Virridy sites were back-calibrated out of order via `prep_virridy_2024_for_cal_app.R`. The back-calibrated values were saved to the finalized calibration RDS but never written back to `verified_directory/`. We recover them here to correct `value_post_cal_corr` for Virridy downstream. Depth and Temperature are not present (no calibration applied to those parameters).
