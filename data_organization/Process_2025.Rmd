...
raw_dataset <- all_datasets%>%
  ...
  mutate(modifications = case_when( is.na(value_post_ver) & !is.na(value_raw) ~ "manual data exclusion",
...
slice_head(n = 1, by = c(DateTime_UTC, SiteID, parameter))
    
    
final_dataset <- raw_dataset %>%
  select(DateTime_UTC, SiteID, parameter, units, value_raw, value_final = value_post_drift_corr, modifications)
...
```{r}
location_meta <- read_csv("data/raw/spatial/metadata/sonde_location_metadata_hydroshare.csv",
