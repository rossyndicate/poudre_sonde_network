---
title: "Processing 2026 Data Pull from HydroVu with Auto Back-Calibration and QA/QC Flagging"
...
mst_start <- ymd_hms("2026-03-01 00:00:00", tz = "America/Denver")
mst_end <- ymd_hms("2026-11-30 23:59:59", tz = "America/Denver")

sites <- c(
  #Upper sites
  #"chd","pfal", "pbr", "sfm",  "pman", "pbd",
  #Lower sites
...
```{r}
base_folder <- here("data","raw", "sensor","manual_data_verification","2026_cycle")
...
log_directory <- here("data","raw", "sensor",  "log_download", "2026")  #Change as needed
...
data_check_plots <- tidy_data%>%
  ...
  map(., function(df) {
    ...
    field_notes <- all_field_notes %>%filter(site == unique(df$site) & year(DT_round) == "2026")%>%
...
#write_rds(calibrated_data, here("data", "raw", "sensor", "manual_data_verification", "2026_cycle","hydro_vu_pull","back_calibration", "calibrated_sensor_data_2026.rds"))
...
```{r}
#calibrated_data_final <- read_rds(here("data", "raw", "sensor", "manual_data_verification", "2026_cycle","hydro_vu_pull","back_calibration", "calibrated_sensor_field_data_finalized_2026.rds"))[["2026"]] # Only extract the 2026 data
...
combined_data <- tidy_data %>% #update to tidy_cal_data when ready to apply back-calibrated data
...
site_order_2026 <- load_site_order( here("data","derived","auto_qaqc_files", "site_order", "PWQN_site_order_2026.yml"))

#Applying Network check (non parrallel so only map used)
final_flags <- intrasensor_flags_list %>%
  map(~network_check(df = ., intrasensor_flags_arg = intrasensor_flags_list, site_order_arg = site_order_2026)) %>%
  rbindlist(fill = TRUE) %>%
  tidy_flag_column() %>%
  split(f = list(.$site, .$parameter), sep = "-") %>%
...
v_final_flags <- final_flags%>%
  ...
  select(c("DT_round", "DT_join", "site", "parameter",
           #"mean_pre_cal", 
           "mean", "units", "n_obs", "spread", "auto_flag", "mal_flag", "sonde_moved","sonde_employed", "season", "last_site_visit", "depth_change"))%>%
           #, "back_cal_performed")) %>%
...
write_parquet(v_final_flags%>%bind_rows(), here(base_folder,"in_progress","raw_data", "raw_data_2026.parquet"))s
```
...
This tool is located in the script `data_organization/drift_correction_2025.Rmd`. Here we are applying the drift corrections identified in the manual verification tool to the data and then saving the output in the "post_verification" folder. 
...
```{r}
drift_corrected_files <- list.files(path = 'data/raw/sensor/manual_data_verification/2025_cycle/post_verification/', pattern = "*.parquet", full.names = TRUE)
