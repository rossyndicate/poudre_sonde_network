...
cal_range_values <- function(sensor_df_arg, end_dt) {
  ...
  # --- Select the option with greater correction potential ---
  # is.finite() guards against NA, NaN, and Inf/-Inf in all cases.
  # A pair is only usable when both endpoints are finite.
  # If both pairs are usable, pick the one with the larger abs(from - to).
  # If the differences are equal (including both zero), default to Option A.
  # If no complete pair is available, mix the best finite individual endpoints
  # (mirrors the old per-endpoint fallback behavior).
  a_valid <- is.finite(from_val_a) && is.finite(to_val_a)
  b_valid <- is.finite(from_val_b) && is.finite(to_val_b)

  if (a_valid && b_valid) {
    diff_a <- abs(from_val_a - to_val_a)
    diff_b <- abs(from_val_b - to_val_b)

    # Direction constraints:
    #   FDOM — biofouling causes downward drift, so the correction must go
    #     upward: to_val >= from_val. Reject any pair where post < pre, even if
    #     the magnitude is large (e.g. a pre > post calibration visit would
    #     otherwise be selected just because |diff| is bigger).
    #   Turbidity — biofouling causes upward drift, so the correction must go
    #     downward: from_val >= to_val. The drifted (pre-cleaning) reading
    #     should always be higher than the clean (post-cleaning) reading; if it
    #     is not, the pair likely reflects a calibration artifact or data error
    #     and should be rejected in favour of the other option.
    a_ok <- (diff_a < 1000) && (if (!is_turb) to_val_a >= from_val_a else from_val_a >= to_val_a)
    b_ok <- (diff_b < 1000) && (if (!is_turb) to_val_b >= from_val_b else from_val_b >= to_val_b)

    if (a_ok && b_ok) {
      # Both directionally valid and reasonable: use bigger correction potential
      if (diff_b > diff_a) {
        return(c(from_val_b, to_val_b))
      } else {
        return(c(from_val_a, to_val_a))
      }
    } else if (b_ok) {
      # Only Option B is directionally valid and reasonable
      return(c(from_val_b, to_val_b))
    } else {
      # Only Option A is valid (or neither) — default to Option A
      return(c(from_val_a, to_val_a))
    }
  } else if (a_valid) {
    return(c(from_val_a, to_val_a))
  } else {
    # Option A is incomplete: fall back per-endpoint (original behavior).
    # This avoids the case where from_val_b ≈ to_val_b (small field-note drift)
    # collapses exp_k's denominator and causes nls to diverge to infinity.
