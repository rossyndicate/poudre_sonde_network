...
cal_range_values <- function(sensor_df_arg, end_dt) {
  ...
  if (a_valid && b_valid) {
    ...
    a_reasonable <- diff_a < 1000
    b_reasonable <- diff_b < 1000
    if (a_reasonable && b_reasonable) {
      # Both reasonable: use the bigger correction potential
      if (diff_b > diff_a) {
      ...
      # Only Option B is within range
      return(c(from_val_b, to_val_b))
    } else {
      # Only Option A is within range, or neither is — default to Option A
      return(c(from_val_a, to_val_a))
    }
  } else if (a_valid) {
    return(c(from_val_a, to_val_a))
...
plot_final_corrections(
  df                  = corrected_data$`archery-Turbidity`,
