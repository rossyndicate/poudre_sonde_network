...
that I would like to explore. Overall, I think that these functions work
as intended. They are not really deciding anything, and I think that a lot of the 
weird results from this correction are due to mislabeling a linear correction as 
an exponential and vice versa. For Turbidity the correction method was almost always
multiplicative, and I think that it worked out real nice for taming extreme variability.
For FDOM the correction method was almost always additive. 
...
There were still instances of drift that I needed to correct, add, or remove. 
...
all of the Turb data to make sure that it looked right, but I did not do the 
same for FDOM. 
...
plot_final_corrections(
  df                  = corrected_data$`udall-Turbidity`,
