...
cal_range_values <- function(sensor_df_arg, end_dt) {
  ...
  if (a_valid && b_valid) {
    ...
    # For FDOM: biofouling always causes downward drift, so the correction must
    # go upward (to_val >= from_val). Reject any option that corrects in the
    # wrong direction regardless of magnitude — this prevents a pre > post
    # calibration visit (e.g. correcting a separate calibration error) from
    # being selected just because |diff| is larger.
    # Turbidity has no direction constraint here.
    a_ok <- (diff_a < 1000) && (if (!is_turb) to_val_a >= from_val_a else TRUE)
    b_ok <- (diff_b < 1000) && (if (!is_turb) to_val_b >= from_val_b else TRUE)
